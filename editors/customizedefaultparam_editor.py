...
from parsers.customizedefaultparam_parser import (
    parse_customizedefaultparam_xfbin,
    save_customizedefaultparam_xfbin,
    make_default_entry,
    SLOT_NAMES,
)
...
class CustomizeDefaultParamEditor(QWidget):
    """Embedded editor for CustomizeDefaultParam.bin.xfbin."""
    ...
    def _build_ui(self):
        ...
        self._show_placeholder(ui_text("ui_customizedefaultparam_open_a_customizedefaultparam_xfbin_file_to_begin_editing"))
    ...
    def _visible_indices(self):
        ...
        for i, e in enumerate(self._entries):
            slot_name = SLOT_NAMES.get(e.get("slot", 0), str(e.get("slot", 0)))
            haystack = f"{e.get('char_code', '')} {e.get('slot', '')} {slot_name}".lower()
    ...
    def _make_entry_button(self, idx):
        ...
        subtitle = QLabel(ui_text("ui_customcardparam_value_value", p0=e.get('char_code', ''), p1=slot_name))
    ...
    def _build_editor(self, idx):
        ...
        self._add_field(grid, 0, 0, "char_code", ui_text("ui_customizedefaultparam_char_code"), e["char_code"], wide=True)
        self._add_field(
            grid, 0, 1, "slot",
            self._label_with_hint(ui_text("ui_customizedefaultparam_slot"), SLOT_NAMES, e["slot"]),
            e["slot"],
        )
        hl.addLayout(grid)
        self._editor_layout.addWidget(hdr)

        self._add_section(ui_text("ui_customizedefaultparam_position_scale"))
        self._add_card_grid([
            ("pos_x", ui_text("ui_customizedefaultparam_pos_x"), self._fmt_float(e["pos_x"], 2)),
            ("pos_y", ui_text("ui_customizedefaultparam_pos_y"), self._fmt_float(e["pos_y"], 2)),
            ("scale", ui_text("ui_customizedefaultparam_scale"), self._fmt_float(e["scale"], 4)),
        ], columns=3)

        self._add_section(ui_text("ui_customcardparam_unknown_fields"))
        self._add_card_grid([
            ("unk", ui_text("ui_customizedefaultparam_unk"), self._fmt_float(e["unk"], 2)),
            ("unk2", ui_text("ui_customizedefaultparam_unk2"), self._fmt_float(e["unk2"], 2)),
        ], columns=3)
    ...
    def _on_field_changed(self, key, text):
        ...
        try:
            ...
            elif key in {"pos_x", "pos_y", "scale", "unk", "unk2"}:
                e[key] = float(text.strip() or "0")
            else:
                e[key] = text.strip()
        except ValueError:
            return
        ...
        for btn, i in self._entry_buttons:
            if i == self._current_index:
                ...
                if len(labels) >= 2:
                    ...
                    labels[1].setText(ui_text("ui_customcardparam_value_value", p0=e.get('char_code', ''), p1=slot_name))
                break
    ...
    def _do_open(self):
        path, _ = QFileDialog.getOpenFileName(
            self, ui_text("ui_customizedefaultparam_open_customizedefaultparam"),
            game_files_dialog_dir(os.path.expanduser("~"), ("CustomizeDefaultParam.xfbin", "CustomizeDefaultParam.bin.xfbin")),
        ...
        try:
            raw, version, entries = parse_customizedefaultparam_xfbin(path)
    ...
    def _do_save(self):
        if not self._filepath or not self._raw_data:
            return
        try:
            save_customizedefaultparam_xfbin(
                self._filepath, self._raw_data, self._version, self._entries
            )
    ...
    def _do_add(self):
        if self._raw_data is None:
            return
        self._entries.append(make_default_entry("1jnt01", 0))
        self._mark_dirty()
    ...
    def _do_del(self):
        ...
        reply = QMessageBox.question(
            ...
            ui_text("ui_customizedefaultparam_delete_entry_value_value_slot_value", p0=self._current_index, p1=e.get('char_code', ''), p2=e.get('slot', 0)),
